...
def _build_seoul_districts():
    ...
    for d in SEOUL_DISTRICTS:
        ...
        add(
            ...
            title=f"{d['name']} 방문 마사지 이용 안내 | 서울 행정동·예약 정보 | 바로GO",
            description=f"서울 {d['name']} 방문 마사지 이용 정보. {d['name']} {len(d['dongs'])}개 행정동 전체 안내, 권역 특성, 예약 가능 시간, 자주 묻는 질문을 정리했습니다.",
...
def _build_gyeonggi_districts():
    ...
    for d in GYEONGGI_DISTRICTS:
        ...
        add(
            ...
            title=f"{d['name']} 방문 마사지 이용 안내 | 경기 | 바로GO",
            description=f"경기 {d['name']} 방문 마사지 이용 정보. {headline} 안내, 권역 특성, 예약 가능 시간, 자주 묻는 질문을 정리했습니다.",
            h1=f"{d['name']} 방문 마사지 이용 안내",
            intro=f'<p class="lede">{d["lede"]}</p>' + _district_hero_cta_html(d["name"]),
            breadcrumbs=[
                ("홈", "/"),
...
def _build_metro_district(parent_slug, parent_name, d, all_in_parent):
    ...
    add(
        ...
        title=f"{d['name']} 방문 마사지 이용 안내 | {parent_name} | 바로GO",
        description=f"{parent_name} {d['name']} 방문 마사지 이용 정보. {headline} 안내, 권역 특성, 예약 가능 시간, 자주 묻는 질문을 정리했습니다.",
        h1=f"{d['name']} 방문 마사지 이용 안내",
        intro=f'<p class="lede">{d["lede"]}</p>' + _district_hero_cta_html(d["name"]),
        breadcrumbs=[
            ("홈", "/"),
